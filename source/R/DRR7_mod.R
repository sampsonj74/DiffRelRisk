...
DRRCI <- function(x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,tr=NA,Y=NA,Z=NA,alpha=0.05,altParam=0,deltaMethod=0,refPop=NA,LRT=0,estRR=0, op=NULL){
...
DRRCI2 <- function(x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,tr=NA,Y=NA,Z=NA,alpha=0.05,altParam=0,deltaMethod=0,
                  refPop=NA,LRT=0,estRR=0){
  ...
  if (!is.na(EST) & LRT==1) try(LB <- boundR.LRT(EST,x1,x2,x3,x4,n1,n2,n3,n4,Y,Z,tr,init.pa,init.p3,init.p4,init.t,init.tp,init.k,init.b,init.R1,init.R2,method.rr=method.rr,altParam=altParam,alpha2=alpha/2,type="LB"))
  if (!is.na(EST) & LRT==1) try(UB <- boundR.LRT(EST,x1,x2,x3,x4,n1,n2,n3,n4,Y,Z,tr,init.pa,init.p3,init.p4,init.t,init.tp,init.k,init.b,init.R1,init.R2,method.rr=method.rr,altParam=altParam,alpha2=alpha/2,type="UB"))
...
optim.range <- function(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="OP",
                        x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=0,allRes=0,s=NA,allParam=NA,paramType=NA,
                        nparam=NA,curIndex=NA,curParam=NA){
  ...
  if (method.rr=="OP" & altParam==0){ 
    if (curParam == "pa")  {
      min.v <-   max(  abs(allParam[paramType=="r"]) * allParam[paramType=="p3" & strata.s==strata.s[curIndex]]/2,MINPROB)
      max.v <-   min(1-abs(allParam[paramType=="r"]) * allParam[paramType=="p3" & strata.s==strata.s[curIndex]]/2,MAXPROB)
    }
    if (curParam == "r") {
      min.v <-   max(c(-2*(1-allParam[paramType =="pa"])/allParam[paramType=="p3"],-2*allParam[paramType =="pa"]/allParam[paramType =="p3"]))
      max.v <-   min(c(    2*allParam[paramType =="pa"]/allParam[paramType=="p3"],2*(1-allParam[paramType =="pa"])/allParam[paramType =="p3"])) 
    }
    if (curParam == "p3")  {
      paJ <- allParam[paramType =="pa" & strata.s==strata.s[curIndex]]
      rJ  <- allParam[paramType=="r"]
      if (rJ != 0) max.v <-   min(c(2*(1-paJ)/abs(rJ),2*paJ/abs(rJ),  MAXP))
      min.v <-  MINP
      if (rJ == 0) max.v <-  MAXP
    }
  }
  ...
  if (method.rr=="OP" & altParam==1){ 
    if (curParam == "t")  { 
      min.v <-   max(      c(abs(allParam[paramType=="r"])/2) )
    ...
    if (curParam == "r")   {
      min.v <-   max( c( -2*(1-allParam[paramType =="t"]*allParam[paramType=="p3"])/allParam[paramType=="p3"], -2*allParam[paramType =="t"]    )) 
  ...
  if (method.rr=="CO") { 
    ...
    if (curParam == "r")  { 
      ...
      max.v <- min(ifelse(tr==1,2*kkk,
                        ifelse(tr==2, 2*(1/exp(Z%*%matrix(bbb,ncol=1))-kkk),5)))
      if (min.v > max.v) {
        min.v <- MINV
        max.v <- MAXV
      }
  ...
  if (method.rr=="COR") {
    if (curParam == "R2" | curParam == "R1") {
      min.v <- 0.000001
      max.v <- max(1/abs(allParam[paramType=="b"]))
    }
    if (curParam == "b") {
      R2R2  <- allParam[length(allParam)]
      R1R1  <- allParam[1]
      bbb   <- allParam[-c(1,curIndex,length(allParam))]
      if (length(bbb)) {
  ...
  if (!length(min.v) || !is.finite(min.v)) min.v <- MINV
  if (!length(max.v) || !is.finite(max.v)) max.v <- MAXV
...
optim.force <-  function(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="OP",
                         x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=0,allRes=0, op=NULL){

  if (!op$fast.c) {
    ret <- optim.force.R(pa=pa,p3=p3,t=t,r=r,k=k,b=b,p4=p4,tp=tp,R1=R1,R2=R2,altParam=altParam,
                       method.rr=method.rr,x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=fix.r,allRes=allRes, op=op)
  } else {
    ret <- optim.force.C(pa=pa,p3=p3,t=t,r=r,k=k,b=b,p4=p4,tp=tp,R1=R1,R2=R2,altParam=altParam,
                         method.rr=method.rr,x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=fix.r,allRes=allRes, 
                         DEBUG=op$DEBUG, initStepSize=1.0, stopTol=1e-3)
  }
...
optim.force.R <-  function(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="OP",x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=0,allRes=0, op=NULL){
  ...
  if (method.rr=="TP")                paramType       <- c(rep("tp",length(tp)),rep("p3",length(p3)),rep("p4",length(p4)),rep("r",length(r)))
...
#print("Init parms:")
#print(allParam)
#fff <- fL.OP(NA,allParam[1:s],allParam[(s+1):(2*s)],allParam[2*s+1],x1,x2,x3,n1,n2,n3)
#print(paste("f0=", fff, sep=""))     
...
#}
  ...
  for (nattempt in 1:10000){
    ...
    for (curIndex in 1:nparam){
...
#if (method.rr=="OP") if(allParam[curIndex] >= min.v & allParam[curIndex] <= max.v)  poss.vals <- c(seq(max(allParam[curIndex]-0.1*mult,min.v),min(allParam[curIndex]+0.1*mult,max.v),length.out=10))
#if (method.rr=="OP") if((allParam[curIndex] < min.v | allParam[curIndex] > max.v))  poss.vals <- c(                   seq(max(allParam[curIndex]-0.1*mult,min.v),min(allParam[curIndex]+0.1*mult,max.v),length.out=10))
      ...
      if (method.rr=="COR") if(allParam[curIndex]  >= min.v & allParam[curIndex] <= max.v) poss.vals <- sort(c(allParam[curIndex],seq(max(allParam[curIndex]-mult2*mult,min.v),min(allParam[curIndex]+mult2*mult,max.v),length.out=10)))
      if (method.rr=="COR") if( (allParam[curIndex] < min.v  | allParam[curIndex] > max.v)) poss.vals <- sort(c(                  seq(max(allParam[curIndex]-mult2*mult,min.v),min(allParam[curIndex]+mult2*mult,max.v),length.out=10)))
      
      
      if (curIndex <   length(allParam) & method.rr=="TP") poss.vals <- sort(c(allParam[curIndex],seq(max(allParam[curIndex]-0.1*mult,0.000001),min(allParam[curIndex]+0.1*mult,0.999999),length.out=10)))
...
#if (fix.r) {
#print(c(nattempt, curIndex, min.v, max.v, min(poss.vals), max(poss.vals), mult))
#} 

      npv       <- length(poss.vals)
      func.vals <- matrix(nrow=npv,ncol=1)
      for (i in 1:npv) {
        allParam[curIndex]=poss.vals[i]
...
#if (fix.r) print(paste("minx=", allParam[curIndex], ", fmin=", min(func.vals), sep=""))

#if (fix.r) {
#print(min(as.vector(func.vals)))
#print(allParam)
#}
 
    } # END: for (curIndex in 1:nparam)
...
#if (!fix.r) {
#adiff <- max(abs(allParam-allParam0))
#print(c(adiff, prevMin, newMin), digits=20)
#}
  ...
  if (mult < 0.1){
    ...
    if (method.rr=="TP"               & allRes==0 & fix.r==1) out <- list("t"=allParam[1:s],"r"=allParam[length(allParam)],"p01"=allParam[(s+1):(2*s)],"p02"=allParam[(2*s+1):(3*s)])
  ...
  list("out"=out,"ok"=ok, retMin=newMin)
} # END: optim.force.R



optim.ideal <- function(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="OP",x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=0,allRes=0, op=NULL){
  s      <- length(x1)
  r.out  <- r
  t.out  <- NA
  pa.out <- rep(NA,s)
...
optim.DSS <- function(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="OP",
                     x1,x2,x3,x4,n1,n2,n3,n4,n,Z,Y,tr,fix.r=0,allRes=0,use.force=0, op=NULL){

  ok    <- 0
  if (use.force < 2) {
    ideal     <- 1
    optim.out <- try(optim.ideal(pa=pa,p3=p3,t=t,r=r,k=k,b=b,p4=p4,tp=tp,R1=R1,R2=R2,altParam=altParam,method.rr=method.rr,
                       x1=x1,x2=x2,x3=x3,x4=x4,n1=n1,n2=n2,n3=n3,n4=n4,n=n,Z=Z,Y=Y,tr=tr,fix.r=fix.r,allRes=allRes, 
                       op=op), silent=TRUE)
    if (!("try-error" %in% class(optim.out)))  ok <- optim.out$ok
  }

  if (!ok) {
      ideal     <- 0
      optim.out <- optim.force(pa=pa,p3=p3,t=t,r=r,k=k,b=b,p4=p4,tp=tp,R1=R1,R2=R2,altParam=altParam,method.rr=method.rr,
                    x1=x1,x2=x2,x3=x3,x4=x4,n1=n1,n2=n2,n3=n3,n4=n4,n=n,Z=Z,Y=Y,tr=tr,fix.r=fix.r,allRes=allRes, op=op)
  }
...
opt.OP1 <- function(r,pa.start,p3.start,x1,x2,x3,n1,n2,n3,forceRoot=0,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=pa.start,p3=p3.start,t=NA,r=r,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="OP",
            x1=x1,x2=x2,x3=x3,x4=rep(0,length(x3)),n1=n1,n2=n2,n3=n3,n4=rep(0,length(x3)),n=NA,Z=NA,Y=NA,tr=NA,
            fix.r=1,allRes=allRes,use.force=use.force, op=op)$out
}


opt.OP2 <- function(r,t.start,p3.start,x1,x2,x3,n1,n2,n3,forceRoot=0,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=p3.start,t=t.start,r=r,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=1,method.rr="OP",
            x1=x1,x2=x2,x3=x3,x4=rep(0,length(x3)),n1=n1,n2=n2,n3=n3,n4=rep(0,length(x3)),n=NA,Z=NA,Y=NA,tr=NA,fix.r=1,
            allRes=allRes,use.force=use.force, op=op)$out
}


opt.TP <- function(r,t.start,p01.start,p02.start,x1,x2,x01,x02,n1,n2,n01,n02,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=p01.start,t=NA,r=r,k=NA,b=NA,p4=p02.start,tp=t.start,R1=NA,R2=NA,altParam=0,method.rr="TP",
            x1=x1,x2=x2,x3=x01,x4=x02,n1=n1,n2=n2,n3=n01,n4=n02,n=NA,Z=NA,Y=NA,tr=NA,fix.r=1,allRes=allRes,
            use.force=use.force, op=op)$out
}



opt.CO <- function(r,k.start,b.start,tr,Z,Y,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=NA,t=NA,r=r,k=k.start,b=b.start,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="CO",
         NA,NA,NA,NA,NA,NA,NA,NA,n=length(Y),Z=Z,Y=Y,tr=tr,fix.r=1,allRes=allRes,use.force=use.force, op=op)$out
}


opt.COR <- function(R1,R2.start,b.start,tr,Z,Y,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=b.start,p4=NA,tp=NA,R1=R1,R2=R2.start,altParam=0,method.rr="COR",
            NA,NA,NA,NA,NA,NA,NA,NA,n=length(Y),Z=Z,Y=Y,tr=tr,fix.r=1,allRes=allRes,use.force=use.force, op=op)$out
}
...
getEst.OP <- function(init.pa=NA,init.t=NA,init.p3,init.r,x1,x2,x3,n1,n2,n3,altParam=0,unCond=0,allRes=0,forceRoot=0,use.force=0, op=NULL){
  optim.DSS(pa=init.pa,p3=init.p3,t=init.t,r=init.r,k=NA,b=NA,p4=NA,tp=NA,R1=NA,R2=NA,altParam=altParam,method.rr="OP",
            x1=x1,x2=x2,x3=x3,x4=rep(0,length(x3)),n1=n1,n2=n2,n3=n3,n4=rep(0,length(x3)),n=NA,Z=NA,Y=NA,tr=NA,fix.r=0,
            allRes=allRes,use.force=use.force, op=op)$out
}

getEst.TP <- function(init.p4,init.tp,init.p3,init.r,x1,x2,x3,x4,n1,n2,n3,n4,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=init.p3,t=NA,r=init.r,k=NA,b=NA,p4=init.p4,tp=init.tp,R1=NA,R2=NA,altParam=0,method.rr="TP",
            x1=x1,x2=x2,x3=x3,x4=x4,n1=n1,n2=n2,n3=n3,n4=n4,n=NA,Z=NA,Y=NA,tr=NA,fix.r=0,allRes=allRes,use.force=use.force, op=op)$out
}


getEst.CO <- function(init.k,init.b,init.r,Y,Z,tr,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=NA,t=NA,r=init.r,k=init.k,b=init.b,p4=NA,tp=NA,R1=NA,R2=NA,altParam=0,method.rr="CO",
            x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,n=length(Y),Z=Z,Y=Y,tr=tr,fix.r=0,allRes=allRes,use.force=use.force, op=op)$out
}

getEst.COR <- function(init.R2,init.b,init.R1,Y,Z,tr,allRes=0,use.force=0, op=NULL){
  optim.DSS(pa=NA,p3=NA,t=NA,r=NA,k=NA,b=init.b,p4=NA,tp=NA,R1=init.R1,R2=init.R2,altParam=0,method.rr="COR",
            x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,n=length(Y),Z=Z,Y=Y,tr=tr,fix.r=0,allRes=allRes,use.force=use.force, op=op)$out
}
...
getZ.OP1 <- function(r,x1,x2,x3,n1,n2,n3,unCond=0,forceRoot=0,sv=NA,use.force=0, op=NULL){
  if (sum(!is.na(sv))==0) {
    sv  <- startVals.OP(x1,x2,x3,n1,n2,n3,r)
  }
  sv2 <- opt.OP1(r,sv$pa,sv$p3,x1,x2,x3,n1,n2,n3,forceRoot=forceRoot,use.force=use.force, op=op)
  Z <- getZ2.OP1(sv2$pa,sv2$p3,r,x1,x2,x3,n1,n2,n3,unCond)
  list("Z"=Z,"sv2"=sv2)
}


getZ.OP2 <- function(r,x1,x2,x3,n1,n2,n3,unCond=0,forceRoot=0,sv=NA,use.force=0, op=NULL){
  if (sum(!is.na(sv))==0) sv  <- startVals.OP(x1,x2,x3,n1,n2,n3,r)
  sv2 <- opt.OP2(r,sv$t,sv$p3,x1,x2,x3,n1,n2,n3,forceRoot=forceRoot,use.force=use.force, op=op)
  Z <- getZ2.OP2(sv2$t,sv2$p3,r,x1,x2,x3,n1,n2,n3,unCond)
  list("Z"=Z,"sv2"=sv2)
}

getZ.TP <- function(r,x1,x2,x01,x02,n1,n2,n01,n02,unCond=0,sv=NA,use.force=0, op=NULL){
  if (sum(!is.na(sv))==0)  {

    sv  <- startVals.TP(x1,x2,x01,x02,n1,n2,n01,n02,r)
  }
  sv2 <- opt.TP(r,sv$t,sv$p01,sv$p02,x1,x2,x01,x02,n1,n2,n01,n02,use.force=use.force, op=op)
  Z <- getZ2.TP(r,sv2$t,sv2$p01,sv2$p02,x1,x2,x01,x02,n1,n2,n01,n02,unCond)
  list("Z"=Z,"sv2"=sv2)
}

getZ.CO <- function(r,tr,Z,Y,unCond=0,sv=NA,use.force=0, op=NULL){
  if (sum(!is.na(sv))==0) sv  <- startVals.CO(r,tr,Z,Y)
  sv2 <- opt.CO(r,sv$k,sv$b,tr,Z,Y,use.force=use.force, op=op)
  ZZ<- getZ2.CO(r,sv2$k,sv2$b,tr,Z,Y,unCond)
  list("Z"=ZZ,"sv2"=sv2)
}

getZ.COR <- function(R1,tr,Z,Y,unCond=0,sv=NA,use.force=0, op=NULL){
  if (sum(!is.na(sv))==0) sv  <- startVals.COR(R1,tr,Z,Y)
  sv2 <- opt.COR(R1,sv$R2,sv$b,tr,Z,Y,use.force=use.force, op=op)
  Z<- getZ2.COR(R1,sv2$R2,sv2$b,tr,Z,Y,unCond)
  list("Z"=Z,"sv2"=sv2)
}

getZ.OP <- function(r,x1,x2,x3,n1,n2,n3,altParam=0,unCond=0,forceRoot=0,sv=NA,use.force=0, op=NULL){
  if (altParam==0) {ZSV <- getZ.OP1(r,x1,x2,x3,n1,n2,n3,unCond,forceRoot=forceRoot,sv=sv,use.force=use.force, op=op)
  Z   <- ZSV$Z
  sv2 <- ZSV$sv2
  }
  if (altParam==1) {ZSV <- getZ.OP2(r,x1,x2,x3,n1,n2,n3,unCond,forceRoot=forceRoot,sv=sv,use.force=use.force, op=op)
  Z   <- ZSV$Z
  sv2 <- ZSV$sv2
  }
  list("Z"=Z,"sv2"=sv2)
...
####
findR.int <- function(r,alpha2,x1,x2,x3,x4,n1,n2,n3,n4,tr,Z,Y,altParam=0,method.rr="OP",unCond=0,sv=NA,use.force=0, op=NULL)    {

  if (method.rr=="OP") out1 <- getZ.OP(r,x1,x2,x3,n1,n2,n3,altParam,unCond,forceRoot=1,sv=sv,use.force=use.force, op=op)
  if (method.rr=="TP") out1 <- getZ.TP(r,x1,x2,x3,x4,n1,n2,n3,n4,unCond,sv=sv,use.force=use.force, op=op)
  if (method.rr=="CO") out1 <- getZ.CO(r,tr,Z,Y,unCond,sv=sv,use.force=use.force, op=op)
  if (method.rr=="COR")out1 <- getZ.COR(r,tr,Z,Y,unCond,sv=sv,use.force=use.force, op=op)
...
boundR2    <- function(EST,x1,x2,x3,x4,n1,n2,n3,n4,tr,Z,Y,R2=NA,alpha2=0.025,type="UB",altParam=0,method.rr="OP",unCond=0, op=NULL)   {
  out       <- NA
  ntry      <- 0
  use.force <- 0
  BADVALUE  <- 99999
  ...
  while (ntry <= 2 & is.na(out)) { 
    ...
    if (type=="UB") max.r    <- EST+CONST
    if (type=="UB") start.r  <- min.r
    if (type=="UB") last.r   <- max.r
    if (type=="LB") min.r    <- EST-CONST
    ...
    mult <- 0.01
    if (abs(EST) <= 0.05 & op$fast == 0) mult <- 0.001
    ...
    for (i in 1:length(poss.r)) {
      aaa=try(out2 <- findR.int(r=poss.r[i],alpha2=alpha2,x1=x1,x2=x2,x3=x3,x4=x4,n1=n1,n2=n2,n3=n3,n4=n4,tr=tr,Z=Z,Y=Y,
              altParam=altParam,method.rr=method.rr,unCond=unCond,sv=sv,use.force=use.force, op=op),silent=TRUE)   
...
#if (type == "LB") print(c(poss.r[i], poss.d[i]))
    ...
    if (reachMax || !aDec || !is.finite(poss.r[i-2]) || !is.finite(poss.r[i])) next
  ...
  if (is.na(out)) {
    if (DEBUG) print("Calling boundR2.force")
    out <- boundR2.force(EST,x1,x2,x3,x4,n1,n2,n3,n4,tr,Z,Y,R2=NA,alpha2=alpha2,type=type,
                         altParam=altParam,method.rr=method.rr,unCond=unCond, op=op) 
  }
...
boundR    <- function(EST,x1,x2,x3,x4,n1,n2,n3,n4,tr,Z,Y,alpha2=0.025, type="UB",altParam=0, method.rr="TP",unCond=0, op=NULL)  {
       out <- boundR2(EST,x1,x2,x3,x4,n1,n2,n3,n4,tr,Z,Y,alpha2=alpha2,type=type,altParam=altParam,
...
boundR.LRT <- function(init.r=NA,x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,Y=NA,Z=NA,tr=NA,init.pa=NA,init.p3=NA,init.p4=NA,
                       init.t=NA,init.tp=NA,init.k=NA,init.b=NA,init.R1=NA,init.R2=NA,method.rr="OP",altParam=0,
                       alpha2=0.05/2,type="LB", op=NULL){

  BADVALUE <- 999999
  qval     <- qchisq(1-2*alpha2,1)
  CONST    <- 5

  comp        <- function(r=NA,x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,Y=NA,Z=NA,tr=NA,s=NA,EST.1=NA,init.pa=NA,init.p3=NA,
                          init.p4=NA,init.t=NA,init.tp=NA,init.k=NA,init.b=NA,R1=NA,init.R2=NA,method.rr="OP",altParam=0, op=NULL) {
    if (method.rr=="OP" & altParam==0) EST.2     <- opt.OP1(r,init.pa,init.p3,x1,x2,x3,n1,n2,n3,forceRoot=1,allRes=1,use.force=1, op=op)
    if (method.rr=="OP" & altParam==1) EST.2     <- opt.OP2(r,init.t,init.p3,x1,x2,x3,n1,n2,n3,forceRoot=1,allRes=1,use.force=1, op=op)
    if (method.rr=="TP")               EST.2     <- opt.TP(r,init.tp,init.p3,init.p4,x1,x2,x3,x4,n1,n2,n3,n4,allRes=1,use.force=1, op=op)
    if (method.rr=="CO")               EST.2     <- opt.CO(r,init.k,init.b,tr,Z,Y,allRes=1,use.force=1, op=op)
    if (method.rr=="COR")              EST.2     <- opt.COR(R1,init.R2,init.b,tr,Z,Y,allRes=1,use.force=1, op=op)
  ...
  if (method.rr=="OP")               EST.1     <- getEst.OP(init.pa,init.t,init.p3,init.r,x1,x2,x3,n1,n2,n3,altParam,unCond=0,allRes=1,use.force=1, op=op)
  if (method.rr=="TP")               EST.1     <- getEst.TP(init.p4,init.tp,init.p3,init.r,x1,x2,x3,x4,n1,n2,n3,n4,allRes=1,use.force=1, op=op)
  if (method.rr=="CO")               EST.1     <- getEst.CO(init.k,init.b,init.r,Y,Z,tr,allRes=1,use.force=1, op=op)
  if (method.rr=="COR")              EST.1     <- getEst.COR(init.R2,init.b,init.R1,Y,Z,tr,allRes=1,use.force=1, op=op)
  
  if (type=="UB" & method.rr != "COR") poss.r <- seq(init.r, init.r+CONST,by= 0.01)
  if (type=="LB" & method.rr != "COR") poss.r <- seq(init.r, init.r-CONST,by=-0.01)
  if (type=="UB" & method.rr == "COR") poss.r <- seq(init.R1, init.R1+CONST,by= 0.01)
  ...
  ok     <- 0
  for (i in 1:npr) {
    if (method.rr=="OP" & altParam==0) v[i] <- comp(r=poss.r[i],x1=x1,x2=x2,x3=x3,x4=NA,n1=n1,n2=n2,n3=n3,n4=NA,Y=NA,Z=NA,tr=NA,s=s,EST.1=EST.1,init.pa=init.pa,init.p3=init.p3,init.p4=NA,init.t=NA,init.tp=NA,init.k=NA,init.b=NA,method.rr="OP",altParam=0, op=op)
    if (method.rr=="OP" & altParam==1) v[i] <- comp(r=poss.r[i],x1=x1,x2=x2,x3=x3,x4=NA,n1=n1,n2=n2,n3=n3,n4=NA,Y=NA,Z=NA,tr=NA,s=s,EST.1=EST.1,init.pa=NA,init.p3=init.p3,init.p4=NA,init.t=init.t,init.tp=NA,init.k=NA,init.b=NA,method.rr="OP",altParam=1, op=op)
    if (method.rr=="TP"              ) v[i] <- comp(r=poss.r[i],x1=x1,x2=x2,x3=x3,x4=x4,n1=n1,n2=n2,n3=n3,n4=n4,Y=NA,Z=NA,tr=NA,s=s,EST.1=EST.1,init.pa=NA,init.p3=init.p3,init.p4=init.p4,init.t=NA,init.tp=init.tp,init.k=NA,init.b=NA,method.rr="TP",altParam=0, op=op)
    if (method.rr=="CO"              ) v[i] <- comp(r=poss.r[i],x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,Y=Y,Z=Z,tr=tr,s=s,EST.1=EST.1,init.pa=NA,init.p3=NA,init.p4=NA,init.tp=NA,init.k=init.k,init.b=init.b,method.rr="CO",altParam=0, op=op)
    if (method.rr=="COR"             ) v[i] <- comp(r=NA,x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,Y=Y,Z=Z,tr=tr,s=s,EST.1=EST.1,init.pa=NA,init.p3=NA,init.p4=NA,init.tp=NA,init.k=NA,init.b=init.b,init.R2=init.R2,R1=poss.r[i],method.rr="COR",altParam=0, op=op)
    av <- abs(v[i])


#if (type == "LB") {
#print(paste("r=", poss.r[i], ", abs(v[i])=", abs(v[i]), sep=""))
#}

    if ((av > qval) && (av < BADVALUE)) {
      ok <- 1
      break
    }
  }
  ...
  if (i > 1) {if (type=="UB") poss.r2 <- seq(poss.r[i-1], poss.r[i],by= 0.001)
              ...
              for (i in 1:npr2) {
                if (method.rr=="OP" & altParam==0) v2[i] <- comp(r=poss.r2[i],x1=x1,x2=x2,x3=x3,x4=NA,n1=n1,n2=n2,n3=n3,n4=NA,Y=NA,Z=NA,tr=NA,s=s,EST.1=EST.1,init.pa=init.pa,init.p3=init.p3,init.p4=NA,init.t=NA,init.tp=NA,init.k=NA,init.b=NA,method.rr="OP",altParam=0, op=op)
                if (method.rr=="OP" & altParam==1) v2[i] <- comp(r=poss.r2[i],x1=x1,x2=x2,x3=x3,x4=NA,n1=n1,n2=n2,n3=n3,n4=NA,Y=NA,Z=NA,tr=NA,s=s,EST.1=EST.1,init.pa=NA,init.p3=init.p3,init.p4=NA,init.t=init.t,init.tp=NA,init.k=NA,init.b=NA,method.rr="OP",altParam=1, op=op)
                if (method.rr=="TP"              ) v2[i] <- comp(r=poss.r2[i],x1=x1,x2=x2,x3=x3,x4=x4,n1=n1,n2=n2,n3=n3,n4=n4,Y=NA,Z=NA,tr=NA,s=s,EST.1=EST.1,init.pa=NA,init.p3=init.p3,init.p4=init.p4,init.t=NA,init.tp=init.tp,init.k=NA,init.b=NA,method.rr="TP",altParam=0, op=op)
                if (method.rr=="CO"              ) v2[i] <- comp(r=poss.r2[i],x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,Y=Y,Z=Z,tr=tr,s=s,EST.1=EST.1,init.pa=NA,init.p3=NA,init.p4=NA,init.tp=NA,init.k=init.k,init.b=init.b,method.rr="CO",altParam=0, op=op)
                if (method.rr=="COR"             ) v2[i] <- comp(r=NA,x1=NA,x2=NA,x3=NA,x4=NA,n1=NA,n2=NA,n3=NA,n4=NA,Y=Y,Z=Z,tr=tr,s=s,EST.1=EST.1,init.pa=NA,init.p3=NA,init.p4=NA,init.tp=NA,init.k=NA,init.b=init.b,init.R2=init.R2,R1=poss.r2[i],method.rr="COR",altParam=0, op=op)
                av2 <- abs(v2[i])

#if (type == "UB") {
#print(paste("r=", poss.r2[i], ", abs(v[i])=", abs(v2[i]), sep=""))
#}

                if ((av2 > qval) && (av2 < BADVALUE)) {
                  ok <- 1
                  break
                }
              }
...
#'
#' This function estimates and provides a confidence interval for the difference between
#' two relative risks when there is a comparator disease. The goal to estimate
#' the difference: p2/p3 - p1/p3, where p1, p2, and p3 are the risks in each population.
#'
#' @param tr The treatment group for each subject (a vector with one value per subject; values are either 0, 1, or 2)
#' @param Y1 The primary outcome for each subject (a vector with one value per subject; values are either 0 or 1)
#' @param Y2 The secondary outcome for each subject (a vector with one value per subject; values are either 0 or 1)
#' @param alpha The two-sided error rate for the confidence interval (i.e. 0.05 for a 95\% confidence interval)
#' @param small.sample This value is added to number of events in all treatment group/outcome with fewer than 5 events
#' @return EST The estimates for the relative risk comparing group 2 to group 0, the relative risk comparing group 1 to group 0, and the difference in relative risks
#' @return LB The lower bounds for the relative risk comparing group 2 to group 0, the relative risk comparing group 1 to group 0, and the difference in relative risks
#' @return UB The upper bounds for the relative risk comparing group 2 to group 0, the relative risk comparing group 1 to group 0, and the difference in relative risks
#' @return VAR The variances for the relative risk comparing group 2 to group 0, the relative risk comparing group 1 to group 0, and the difference in relative risks
#' 
#' @examples tr = rep(c(0,1,2),each=100)
#' @examples Y1  = c(rep(1,30),rep(0,70),rep(1,10),rep(0,90),rep(1,20),rep(0,80))
#' @examples Y2  = rbinom(300,1,0.3)
#' @examples DRRCI.comp(tr=tr,Y1=Y1,Y2=Y2)
#' @export
#'


DRRCI.comp = function(tr,Y1,Y2,alpha=0.05,small.sample=0){
  ####Perform direct standardization (to a population similar to n0)
  calcTerms = function(tr,Y1,Y2,v=0,ss=0){
    n0        = sum(tr==v & !is.na(tr) & !is.na(Y1) & !is.na(Y2))
    X0        = sum(Y1[ tr==v & !is.na(tr) & !is.na(Y1) & !is.na(Y2)])
    if(X0 <= 5)  X0 = X0+ss
    X0A       = sum(Y2[tr==v & !is.na(tr) & !is.na(Y1) & !is.na(Y2)])
    if(X0A <= 5) X0 = X0A+ss
    X0.both   = sum((Y2*Y1)[tr==v & !is.na(tr) & !is.na(Y1) & !is.na(Y2)])
    p0.hat    = X0/n0
    v0.hat    = n0*p0.hat*(1-p0.hat)
    p0A.hat   = X0A/n0
    v0A.hat   = n0*p0A.hat*(1-p0A.hat)
    c0.hat    = n0*(X0.both/n0 - p0.hat*p0A.hat)
    out       = c(X0,X0A,v0.hat,v0A.hat,c0.hat)
    out}
  terms0    = calcTerms(tr,Y1,Y2,0,0)
  terms1    = calcTerms(tr,Y1,Y2,1,0)
  terms2    = calcTerms(tr,Y1,Y2,2,0)
  RR1       =(terms1[1]*terms0[2])/(terms1[2]*terms0[1])
  RR2       =(terms2[1]*terms0[2])/(terms2[2]*terms0[1])
  terms0    = calcTerms(tr,Y1,Y2,0,small.sample)
  terms1    = calcTerms(tr,Y1,Y2,1,small.sample)
  terms2    = calcTerms(tr,Y1,Y2,2,small.sample)
  var1      = (terms0[2]/(terms1[2]*terms0[1]))^2*terms1[3]+(terms1[1]/(terms1[2]*terms0[1]))^2*terms0[4] +
    ((terms1[1]*terms0[2])/(terms1[2]^2*terms0[1]))^2*terms1[4]+((terms1[1]*terms0[2])/(terms1[2]*terms0[1]^2))^2*terms0[3] -
    2*(terms0[2]/(terms1[2]*terms0[1]))*(terms1[1]*terms0[2])/(terms1[2]^2*terms0[1])*terms1[5]-
    2*(terms1[1]/(terms1[2]*terms0[1]))*(terms1[1]*terms0[2])/(terms1[2]*terms0[1]^2)
  var2      = (terms0[2]/(terms2[2]*terms0[1]))^2*terms2[3]+(terms2[1]/(terms2[2]*terms0[1]))^2*terms0[4] +
    ((terms2[1]*terms0[2])/(terms2[2]^2*terms0[1]))^2*terms2[4]+((terms2[1]*terms0[2])/(terms2[2]*terms0[1]^2))^2*terms0[3] -
    2*(terms0[2]/(terms2[2]*terms0[1]))*(terms2[1]*terms0[2])/(terms2[2]^2*terms0[1])*terms2[5]-
    2*(terms2[1]/(terms2[2]*terms0[1]))*(terms2[1]*terms0[2])/(terms2[2]*terms0[1]^2)
  c12       = (terms1[1]/(terms1[2]*terms0[1]))*(terms2[1]/(terms2[2]*terms0[1]))*terms0[4]+
    ((terms1[1]*terms0[2])/(terms1[2]*terms0[1]^2))*((terms2[1]*terms0[2])/(terms2[2]*terms0[1]^2))*terms0[3]-
    (terms1[1]/(terms1[2]*terms0[1]))*((terms2[1]*terms0[2])/(terms2[2]*terms0[1]^2))*terms0[5] -
    ((terms1[1]*terms0[2])/(terms1[2]*terms0[1]^2))*(terms2[1]/(terms2[2]*terms0[1]))*terms0[5]
  zalpha    = qnorm(1-alpha/2)
  EST    = matrix(c(RR2,RR1,RR2-RR1),nrow=1)
  LB     = matrix(c(RR2-1.96*sqrt(var2),RR1-1.96*sqrt(var1),RR2-RR1-1.96*sqrt(var1+var2-2*c12)),nrow=1)
  UB     = matrix(c(RR2+1.96*sqrt(var2),RR1+1.96*sqrt(var1),RR2-RR1+1.96*sqrt(var1+var2-2*c12)),nrow=1)
  var    = matrix(c(var2,var1,var1+var2-2*c12),nrow=1)
  colnames(EST)=colnames(LB)=colnames(UB)=colnames(var)=c("RR(Treat 2)","RR(Treat 1)","Diff RR")
  out <- list("EST"=EST,"LB"=LB,"UB"=UB,"VAR"=var)
  out
}
